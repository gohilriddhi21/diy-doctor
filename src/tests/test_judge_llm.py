...
def test_success(query_engine, judge_llm_manager, query):
    ...
    faithfulness_score = judge_llm_manager.evaluate_faitfulness(judge_llm, full_response)
    relevancy_score = judge_llm_manager.evaluate_relevancy(judge_llm, query, full_response)
    verification = judge_llm_manager.verify_suggestions(judge_llm, query, full_response)
    print(response_text)
...
def main():
    ...
    judge_llm_manager = JudgeDefault("qwen/qwen-turbo", 512, 4096)
