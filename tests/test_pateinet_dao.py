...
class TestPatientDAO(unittest.TestCase):
    ...
    def test_get_patient_record_failure(self):
        ...
        records = self.patient_dao.__get_patient_by_id__(non_existent_patient_id, "patient_data")
        self.assertEqual(len(records), 0)

    def test_get_patient_records_from_all_collections(self):
        existent_patient_id = "1"
        records = self.patient_dao.get_patient_records_from_all_collections(existent_patient_id)
